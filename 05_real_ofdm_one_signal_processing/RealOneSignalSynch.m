...
%   - компенсация фазы и остаточной частотной отстройки
...
% 20 ppm - цифра от офф разработчика-контрибютера hackrf
%
...
% Компенсация + FFT для поулчения модуляционных символов
bpskBeforeCompensation = [ Constellate_From_LTS(TwoLTS(1 : 64)), ...
                           Constellate_From_LTS(TwoLTS(65 : 128)) ];

bpskAfterCompensation  = [ Constellate_From_LTS(TwoLTS(1 : 64)   .* exp(-1i * estPO)), ...
                           Constellate_From_LTS(TwoLTS(65 : 128) .* exp(-1i * estPO)) ];

if strcmp(ps_graph_mode, 'display')
	Graph_PhaseSynch(bpskBeforeCompensation,  bpskAfterCompensation);
end
...
fprintf('  6) Phase Offset Est:  fi == %.3f рад == %.1f град\n', ...
        estPO, estPO * 180 / pi);
