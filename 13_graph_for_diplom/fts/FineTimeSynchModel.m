...
EbNo        = 10; %[2, 4, 8]; % дБ
time_offset = 200;
deltaF      = 0; %[0, 5 * 10^3, 15 * 10^3]; % Гц, частотная отстройка
% deltaF = 50 * 10^3;
...
segmentLen = time_offset + 160 + 32 + 64  + 200; % кол-во отсчётов для которых выполняется алгоритм FTS; // 0 - значит для всех отсчётов сгенерированного сигнала
...
figure;
hold on;
plot(O_sample, metric,  'LineWidth', 2);
% stem(ideal_peak_index, 0.5); % первый отсчёт преамбулы % !!!! И С П Р А В И Т Ь
% stem(estFTO, 0.6); % оценка первого отсчёта преамбулы
hold off;
grid on;
...
x_label_2 = '$ n $';
xlabel(x_label_2, 'FontSize', 16, 'Interpreter', 'latex');  
grid on;
y_label_2 = '$ Cross Corr $';
ylabel(y_label_2, 'FontSize', 16, 'Interpreter', 'latex');  
% 	title('Signal Detection');
% 	legend('\it m_{n}', '\it m_{n} > Thr');
ax = gca;
ax.YLim = [0, 0.5];
ax.XLim = [350, 500];
% ax.XLim = [300, 450];
ax.Box = 'on';
% ax.Children = [line0, line1];